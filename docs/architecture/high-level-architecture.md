...
**Deployment note:** the structure above — three services, two trust boundaries — is what the codebase implements and what this document describes throughout. The real production deployment doesn't map onto it one-to-one: `gateway` and `ml-backend` are built and run as a single Render Web Service (`combined/Dockerfile`, both processes under `supervisord`), with the `X-Internal-Api-Key` boundary between them now crossing `127.0.0.1` inside one container rather than a network call between two independently-addressable services. `InternalApiKeyMiddleware` still runs, unchanged, on every request — the trust boundary itself isn't removed, just no longer the *only* thing standing between `ml-backend` and an arbitrary caller, since `ml-backend` is no longer independently network-reachable at all in this topology. Frontend and MongoDB connectivity are unaffected. Full reasoning: [ADR-0007](adr/ADR-0007-combined-deployment-topology.md).
...
- [`docs/architecture/adr/ADR-0007-combined-deployment-topology.md`](adr/ADR-0007-combined-deployment-topology.md) — why the real production deployment doesn't match this document's structural diagram one-to-one
