...
- ~~`Cross-Origin-Opener-Policy` on the frontend relaxed to `same-origin-allow-popups` for Firebase's `signInWithPopup`~~ ~~— moot: Google/GitHub sign-in was switched from `signInWithPopup` to `signInWithRedirect` (mobile popup-lifecycle reliability; see `frontend/src/context/AuthContext.jsx`'s `getRedirectResult` handling), so there's no popup window left whose closure needs detecting. `frontend/vercel.json` and `vite.config.js`'s dev-server headers were both tightened back to the stricter `Cross-Origin-Opener-Policy: same-origin` default.~~ **Reverted back to `signInWithPopup`**: `signInWithRedirect`'s cross-origin continuation depends on `/__/firebase/init.json` being served at the Firebase project's `authDomain` (`amr-insight.firebaseapp.com`) — which only happens automatically when that domain is itself hosted on Firebase Hosting. This app is hosted on Vercel, so the endpoint 404s and the redirect flow hangs indefinitely — confirmed via a real reproduction (Network tab) and against [Firebase's own redirect-best-practices doc](https://firebase.google.com/docs/auth/web/redirect-best-practices), which explicitly names "host with a service other than Firebase Hosting" as requiring `signInWithPopup()` instead. This is a structural mismatch between auth method and hosting setup, not something any other fix could have worked around. `Cross-Origin-Opener-Policy` is back to `same-origin-allow-popups` in both `frontend/vercel.json` and `vite.config.js`'s dev-server headers, and `getRedirectResult`/the redirect-completion effect have been removed from `AuthContext.jsx` entirely. **The CSP additions Firebase Auth needs are still real and still only locally verified**: `connect-src` still needs `identitytoolkit.googleapis.com`/`securetoken.googleapis.com` (Firebase Auth's REST backend, required for any provider, popup or redirect, to work under a restrictive CSP) — grounded in Firebase's own documented behavior, confirmed working in local dev, but **not yet verified** against a real production CSP-enforcing host. Verify once a host's real, deployed CSP is exercised end-to-end, before assuming this is complete.
