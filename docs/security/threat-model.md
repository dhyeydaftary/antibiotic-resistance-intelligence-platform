...
> **Updated again (2026-08-17):** the gateway suite now stands at 113 tests, all passing (the 6 pre-existing environmental failures noted above are resolved). The growth since the Firebase-migration count above reflects real feature and security work in the interim — the production deployment itself (see [ADR-0007](../architecture/adr/ADR-0007-combined-deployment-topology.md)), further CORS/security-header/rate-limit regression coverage, and general test-suite maintenance — not a one-time bulk addition. The Django suite, tracked separately, stands at 239 tests, also all passing. See `gateway/tests/` and `ml-backend/predictor/tests/` for the current suites directly rather than trusting this number indefinitely — it will drift again.
...
- `/resend-otp`'s previously distinguishable `ALREADY_VERIFIED` response is moot as of the [Firebase migration (ADR-0005)](../architecture/adr/ADR-0005-firebase-auth-migration.md); the route no longer exists.
- `CSRF_COOKIE_SECURE` being moot is confirmed, with evidence: DRF's `APIView.as_view()` unconditionally wraps every view in `csrf_exempt` (verified directly against the installed `djangorestframework` source), and this app's `DEFAULT_AUTHENTICATION_CLASSES: []` means no `SessionAuthentication` is ever active to reintroduce a CSRF check on top of that exemption. `InternalApiKeyMiddleware` is the real trust boundary here, not cookies.
...
- Several settings were deliberately placeholder/deploy-time-only until a real hosting platform was chosen: the gateway's `trust proxy` value (currently `1`, a single-hop assumption — still applicable, since the combined Render service is still a single hop behind Render's own proxy), the frontend's `vercel.json` CSP `connect-src` (**resolved** — filled in with the real origin, first the standalone gateway's, then the combined Render service's origin after the [ADR-0007](../architecture/adr/ADR-0007-combined-deployment-topology.md) deployment merge), and HSTS `preload` (still deliberately excluded — committing to browser preload lists is hard to reverse and ties the commitment to a specific domain, even though a real production domain now exists).
- `Cross-Origin-Opener-Policy` on the frontend is `same-origin-allow-popups`, required for Firebase's `signInWithPopup`. Google/GitHub sign-in was briefly switched to `signInWithRedirect` for mobile popup-lifecycle reliability, then reverted the same day: `signInWithRedirect`'s cross-origin continuation depends on `/__/firebase/init.json` being served at the Firebase project's `authDomain` (`amr-insight.firebaseapp.com`), which only happens automatically when that domain is itself hosted on Firebase Hosting. This app is hosted on Vercel, so the endpoint 404s and the redirect flow hangs indefinitely — confirmed via a real reproduction (Network tab) and against [Firebase's own redirect-best-practices doc](https://firebase.google.com/docs/auth/web/redirect-best-practices), which explicitly names "host with a service other than Firebase Hosting" as requiring `signInWithPopup()` instead. This is a structural mismatch between auth method and hosting setup, not something any other fix could have worked around. **The CSP additions Firebase Auth needs** — `connect-src`'s `identitytoolkit.googleapis.com`/`securetoken.googleapis.com` (Firebase Auth's REST backend, required for any provider, popup or redirect, to work under a restrictive CSP) — are **verified (2026-08-17)**: multiple successful end-to-end sign-ins, including a full Google OAuth flow, completed against the real, deployed production CSP on the live domain (Vercel-hosted frontend, combined Render backend per ADR-0007). Confirms these entries are both necessary and sufficient under actual browser CSP enforcement, not just local dev.
