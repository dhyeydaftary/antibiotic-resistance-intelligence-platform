...
- ~~`Cross-Origin-Opener-Policy` on the frontend relaxed to `same-origin-allow-popups` for Firebase's `signInWithPopup`~~ — moot: Google/GitHub sign-in was switched from `signInWithPopup` to `signInWithRedirect` (mobile popup-lifecycle reliability; see `frontend/src/context/AuthContext.jsx`'s `getRedirectResult` handling), so there's no popup window left whose closure needs detecting. `frontend/vercel.json` and `vite.config.js`'s dev-server headers were both tightened back to the stricter `Cross-Origin-Opener-Policy: same-origin` default. **The CSP additions Firebase Auth needs are still real and still only locally verified**: `connect-src` still needs `identitytoolkit.googleapis.com`/`securetoken.googleapis.com` (Firebase Auth's REST backend, required for any provider, redirect or not, to work under a restrictive CSP) — grounded in Firebase's own documented behavior, confirmed working in local dev, but **not yet verified** against a real production CSP-enforcing host. `signInWithRedirect` itself navigates the top-level document to the provider, which isn't governed by `connect-src`/`frame-src` the way `fetch`/popup calls are, so it shouldn't need further CSP entries for the provider's own domain — but that's reasoning from Firebase's documented behavior, not yet confirmed against a real deployed host either. Verify both once a host is chosen, before assuming this is complete.
